...
export default {
  ...
  mounted: function(){
    ...
    let index=Math.max(this.modules.findIndex((x)=>x.name==this.$route.name, this),0);
    this.currentModule=this.modules[index];
    if(!this.currentModule.closable){
       this.drawer=!this.currentModule.closable;
    }

    this.version=process.env.VUE_APP_VERSION + "  "+ process.env.VUE_APP_COMMITMESSAGE;//'v' +( process.env.VERCEL_GIT_COMMIT_REF ?? '??') +'.'+  (process.env.VERCEL_GIT_COMMIT_MESSAGE ?? '??');
  ...
  methods:{
    ...
    moduleChanged: async function (item){
      if(this.currentModule.name!=this.$route.name){
        this.$router.push({ name: this.currentModule.name, params: { title: this.currentModule.label } });
